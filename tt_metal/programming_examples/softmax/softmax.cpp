...
int main(int argc, char** argv) {
    ...
    CoreRangeSet cores(CoreRange({0, 0}, {rectangle_width - 1, rectangle_height - 1}));
    size_t rest = N_ROWS_TILES % max_cores_x;
    if (N_ROWS_TILES > max_cores_x && N_ROWS_TILES < max_cores && rest > 0) {
        auto merge_cores = CoreRange({0, rectangle_height}, {rest - 1, rectangle_height});
        std::vector<CoreRange> merge_vec = {merge_cores};
        cores = cores.merge(merge_vec);
    }
    ...
    auto reader_kernel_id = CreateKernel(
        program,
        "tt_metal/programming_examples/softmax/kernels/dataflow/reader.cpp",
        cores,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc = NOC::RISCV_1_default});
    auto writer_kernel_id = CreateKernel(
        program,
        "tt_metal/programming_examples/softmax/kernels/dataflow/writer.cpp",
        cores,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_0, .noc = NOC::RISCV_0_default});
    auto softmax_kernel_id = CreateKernel(
        program, "tt_metal/programming_examples/softmax/kernels/compute/softmax.cpp", cores, tt_metal::ComputeConfig{});
