...
void MAIN {
    ...
    for (uint32_t i = 0; i < n_rows_tile_per_core; i++) {
        ...
        }
        {
            ...
            reduce_init_delta<false, PoolType::MAX, ReduceDim::REDUCE_ROW>(cb_output, cb_row, cb_bcast_scaler);
        ...
        }
        {
            ...
            sub_bcast_cols_init_short(cb_row, cb_reduce);
            // For each block of 8 tiles
            for (uint32_t j = 0; j < n_cols_tiles; j += n_tiles_block) {
                acquire_dst();
        ...
        }
        {
            ...
            acquire_dst();
            cb_reserve_back(cb_reduce, 1);
            reduce_init_delta<false, PoolType::SUM, ReduceDim::REDUCE_ROW>(cb_output, cb_row, cb_bcast_scaler);
        ...
        }
        {
            ...
            cb_wait_front(cb_reduce, 1);
            mul_bcast_cols_init_short(cb_row, cb_reduce);
            // For each block of 8 tiles
            for (uint32_t j = 0; j < n_cols_tiles; j += n_tiles_block) {
                acquire_dst();
