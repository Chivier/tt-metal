...
def test_scatter_floating_point(input_shape, dim, index_and_src_shape, input_dtype, device):
    ...
    torch_result = torch.scatter_(torch_input, dim, index=torch_index, src=torch_src)
    ttnn_result = ttnn.experimental.scatter(ttnn_input, dim, ttnn_index, ttnn_src)

    assert ttnn.to_torch(ttnn_result).shape == torch_result.shape
    assert ttnn.to_torch(ttnn_result).dtype == torch_result.dtype
    assert_with_pcc(torch_result, ttnn.to_torch(ttnn_result))

    torch_output_preallocated = torch.zeros(torch_input.shape)
    ttnn_output_preallocated = ttnn.zeros(ttnn_input.shape)

    torch_preallocated_result = torch.scatter_(
        torch_input, dim, index=torch_index, src=torch_src, out=torch_output_preallocated
    )
    ttnn_preallocated_result = ttnn.experimental.scatter(
        ttnn_input, dim, index=ttnn_index, src=ttnn_src, out=ttnn_output_preallocated
    )

    assert ttnn.to_torch(ttnn_preallocated_result).shape == torch_preallocated_result.shape
    assert ttnn.to_torch(ttnn_preallocated_result).dtype == torch_preallocated_result.dtype

    assert_with_pcc(torch_preallocated_result, ttnn.to_torch(ttnn_preallocated_result))
