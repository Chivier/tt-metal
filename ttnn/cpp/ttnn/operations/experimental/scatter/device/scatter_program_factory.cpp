...
ScatterProgramFactory::cached_program_t ScatterProgramFactory::create(
    ...
    const uint32_t input_tiles = 2 * Wt_input;
    constexpr uint32_t index_tiles = 2;
    constexpr uint32_t src_tiles = 2;
    ...
    auto reader_kernel =
        create_kernel(program, reader_kernel_path, CoreRangeSet{core}, ReaderDataMovementConfig{}, compile_time_args);

    auto writer_kernel =
        create_kernel(program, writer_kernel_path, CoreRangeSet{core}, WriterDataMovementConfig{}, compile_time_args);
