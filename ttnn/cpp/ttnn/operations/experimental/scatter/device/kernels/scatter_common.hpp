...
tile_guts_gen(uint8_t) tile_guts_gen(uint16_t) tile_guts_gen(uint32_t)

    template <bool is_dram>
    FORCE_INLINE InterleavedAddrGenFast<is_dram> make_addr_gtor(const uint32_t& cb, const uint32_t& base_addr) {
    return {.bank_base_address = base_addr, .page_size = get_tile_size(cb), .data_format = get_dataformat(cb)};
}

FORCE_INLINE std::size_t calculate_ht_offset_for_core(const uint32_t& core_id, ) {
    //
}
